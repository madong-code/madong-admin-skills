...
if ($targets -contains 'cursor') {
    ...
    foreach ($skill in $skills) {
        ...
        New-Item -ItemType Directory -Force -Path $skillDir | Out-Null
        Copy-Item -Path $skill.FullName -Destination (Join-Path $skillDir 'rule.mdc') -Force
...
if ($targets -contains 'trae') {
    ...
    foreach ($skill in $skills) {
        ...
        if ($content -match '(?s)^---\r?\n(.*?)\r?\n---\r?\n(.*)$') {
            $frontmatter = $matches[1]; $body = $matches[2]
            $desc = ''; $globsList = @()
            ...
            if ($globsList.Count -gt 0) { $newFront += 'globs: "' + ($globsList -join ', ') + '"' }
...
if ($targets -contains 'copilot') {
    ...
    $null = $sb.AppendLine("# Madong Skills Instructions")
    $null = $sb.AppendLine("")
    foreach ($skill in $skills) {
        ...
        $null = $sb.AppendLine(""); $null = $sb.AppendLine($content.Trim()); $null = $sb.AppendLine("")
    }
